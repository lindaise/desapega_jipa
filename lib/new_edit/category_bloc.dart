...
class CategoryBloc extends BlocBase {
  ...
  Stream<String> get outTitle => _titleController.stream.transform( // validando campo de titulo
  ...
  void setImage(File file){ // --------- imagem da galeria chamado pelo category_dialog
    image = file;
    _imageController.add(file);
  }

  void setTitle(String title){ // salvando titulo atual e mandando para o controlador
    this.title = title;
  ...
  Future saveData() async {
    if(image == null && category != null && title == category.data["title"]) return; // se nada mudou manda para o firebase
    ...
    if(category == null){
      await Firestore.instance.collection("anuncios").document(title.toLowerCase())
