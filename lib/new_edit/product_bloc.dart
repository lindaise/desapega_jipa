...
class ProductBloc extends BlocBase {
  ...
  ProductBloc({this.categoryId, this.product}){
    if(product != null){
      ...
      unsavedData["it-works"] = List.of(product.data["it-works"]);

      _createdController.add(true);
    } else {
      unsavedData = {
        "title": null, "description": null, "price": null, "images": [], "it-works": []
      };
  ...
  Future<bool> saveProduct() async {
    ...
    try {
      if(product != null){
        ...
        DocumentReference dr = await Firestore.instance.collection("anuncios").document(categoryId).
